...
#include "../Lab7/SciOp.hpp"
#include "../Lab7/SciRand.hpp"
#include "../Lab7/PrecisionOp.hpp"
#include "../Lab7/PrecisionRand.hpp"
#include "../Lab7/ScientificFactory.hpp"
#include "../Lab7/PrecisionFactory.hpp"
...
int main() {
	...
	while (input != "exit") {
		...
		if (input == "exit") {
			...
			// Calculations must start with a singl number, so the first entry is a special case which creates an initial command which is essentially a composite tree with a single node.
			if (!menu->initialized()) {
				if (is_integer(input)) {
			...
			// Once the system has been initialized, the rest of the entries should start with an operator and be followed by an operand (separated by a space), which adds a new operator and operand on top of the tree that has been created previously.
			else {
				Command* cmd = parse_command_input(menu->get_command(), factory, input);
				if (cmd == nullptr) {
					cout << "Invalid command, current command is..." << endl;
				}
				else {
		...
		// After every command is entered we want to print what the most recent good command contains, or print a special "Empty" message when the user calls undo enough times to go back to the beginning of the program or when invalid initial commands are entered.
		if (!menu->initialized()) {
			cout << "Empty" << endl;
		}
		else {
